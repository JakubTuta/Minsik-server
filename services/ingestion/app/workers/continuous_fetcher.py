...
async def run_continuous_ol_fetch(shutdown_event: asyncio.Event) -> None:
    ...
    while not shutdown_event.is_set():
        ...
        try:
            ...
            logger.info(f"Starting continuous OL fetch: {count} books at offset {offset}")
            ...
            logger.info(f"Continuous OL fetch done: {result['processed']} processed, {result['successful']} successful, {result['failed']} failed")
...
async def run_continuous_gb_fetch(shutdown_event: asyncio.Event) -> None:
    ...
    while not shutdown_event.is_set():
        ...
        try:
            ...
            logger.info(f"Starting continuous GB fetch: {count} books at offset {offset}")
            result = await app.workers.ingestion_worker.process_ingestion_job(
                job_id, count, "google_books", "en", offset
            )

            redis_client.set(_GB_OFFSET_KEY, offset + count)
            logger.info(f"Continuous GB fetch done: {result['processed']} processed, {result['successful']} successful, {result['failed']} failed")
