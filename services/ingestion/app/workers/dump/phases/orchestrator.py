...
import app.workers.dump.phases.authors
import app.workers.dump.phases.editions
import app.workers.dump.phases.ratings
import app.workers.dump.phases.reading_log
import app.workers.dump.phases.wikidata
import app.workers.dump.phases.works
import app.workers.dump.state
import redis
...
async def run_import_dump(job_id: str, redis_client: redis.Redis) -> None:
    ...
    saved_state = app.workers.dump.state.get_job_state(redis_client)
    ...
    def _set_status(msg: str) -> None:
        try:
            ...
            redis_client.set(
                "dump_import_status", msg, ex=app.workers.dump.state._REDIS_JOB_STATE_TTL
            )
        except Exception:
            pass
    ...
    try:
        ...
        if 1 not in completed:
            ...
            await app.workers.dump.downloader.download_file(
                f"{base_url}/ol_dump_authors_latest.txt.gz",
                str(phase_files[1]),
            )
            _set_status("Phase 1/6: processing authors")
            authors_count = await app.workers.dump.phases.authors.process_authors_dump(str(phase_files[1]))
        ...
        if 2 not in completed:
            if app.config.settings.dump_wikidata_enabled:
                ...
                wikidata_count = await app.workers.dump.phases.wikidata.process_wikidata_enrichment()
        ...
        if 3 not in completed:
            ...
            works_count = await app.workers.dump.phases.works.process_works_dump(str(phase_files[3]))
        ...
        if 4 not in completed:
            if app.config.settings.dump_editions_enabled:
                ...
                async with app.models.AsyncSessionLocal() as session:
                    known_works_filter = await app.workers.dump.parsers.build_known_works_filter(session)
                ...
                editions_stats = await app.workers.dump.phases.editions.process_editions_dump(
        ...
        if 5 not in completed:
            if app.config.settings.dump_ratings_enabled:
                ...
                ratings_count = await app.workers.dump.phases.ratings.process_ratings_dump(str(phase_files[5]))
        ...
        if 6 not in completed:
            if app.config.settings.dump_reading_log_enabled:
                ...
                reading_log_count = await app.workers.dump.phases.reading_log.process_reading_log_dump(
                    str(phase_files[6])
                )
        ...
        try:
            ...
            if completed == {1, 2, 3, 4, 5, 6}:
                app.workers.dump.state.clear_job_state(redis_client)
        except Exception:
            pass
